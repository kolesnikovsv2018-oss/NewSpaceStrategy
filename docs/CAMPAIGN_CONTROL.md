...
- Статус на 2026-09-10: **S3.30 — историческое проектирование; S3.31 реализовал чистый API раздела3, S3.32 — чистый кодек/миграцию раздела7**. Run и новый кодек не импортированы игровым entry. Игра остаётся S3.29/ручной AI/save1/1. **Разделы4–6 (UI, scheduler, pending) и интеграция run-слота всё ещё будущие**; автоматического red-хода нет. S3.32:446 новых тестов =415 основных/26 fault-only/5 integration; основной агент повторно подтвердил2449/39 файлов/282 сценовых, strict/build71 модуль, `main-9990d074`,1678.14 kB/gzip397.33 kB, прежний warning. Браузер не запускался — pure codec. Следующий только S3.33: репозиторий того же слота без UI/autoscheduling. Заголовки сохранены ради существующих якорей.
...
**Чистый кодек и строгая миграция реализованы S3.32; исторический заголовок сохранён ради якоря. Репозиторий/сцена/пауза/Resume в этом разделе остаются будущими.** Выбран **schemaVersion2/rulesVersion1**: структура документа меняется, механика/формулы нет. Будущий run-репозиторий использует тот же `orion_campaign_v1` — стабильный ключ слота, не указатель поддерживаемой версии. Не создавать второй ключ, не читать/писать библиотеку. Старый клиент отказывает, а не теряет control при round-trip.
...
**Будущий S3.33:** отдельный `CampaignRunSaveManager`, optional `StoragePort`, тот же ключ; constructor без IO/lazy global внутри операций. Load один get/null SAVE_NOT_FOUND/read или getter exception STORAGE_READ_FAILED/иначе новый decode. Save сначала новый encode **до getter**, потом один set, getter/write exception STORAGE_WRITE_FAILED, success после возврата set; фиксированные сообщения. Старый менеджер неизменен. Без cache/read-before-save/remove/rollback/fallback, библиотечных операций/UI/autoscheduling. После отдельного внедрения сцена использует только run-менеджер, не выбирает reader по содержимому; legacy state-only API остаётся1/1 и отвергает2 без потери control.
...
Decode старого1/1 не пишет upgraded-документ; только явный encode нового run создаёт2/1 **даже для local**. Подтверждение занятого слота и capture целого run — будущая интеграция caller/сцены, не действие кодека. Будущий отказ записи сохраняет предыдущие bytes по контракту atomic set/throw порта; не обещать откат на старый клиент. Кап/UTF-8/независимые design snapshots/ID/порядок/fuel/FIFO/cross-state сохраняются. Строгий1/1 ровно5MB может успешно мигрировать в памяти, но явный encode2 превысит cap из-за control; это SAVE_TOO_LARGE, не обрезка. Нет гарантии browser quota/вместимости любых400 snapshots.

## 8. Требуемая приёмка будущей реализации

Это исходная матрица **полной будущей интеграции controller**, не список полностью выполненных проверок. S3.31 закрыл чистый control/run (210 тестов, раздел10); S3.32 — чистый кодек/миграцию (446 тестов, раздел11). Игра и282 сценовых остаются S3.29; общих тестов теперь2449/39 файлов. JSON round-trip целого run уже проверен, но run-слот, отложенная передача, paused/Resume и browser controller ещё нет.
...
**Следующий только S3.33:** `CampaignRunSaveManager` по разделу7: тот же ключ/optional StoragePort, constructor без IO/lazy global, load один get→новый decode, save encode до getter→один set, fixed codes/неизменный старый менеджер. Регрессии новых менеджеров/read-write отказов/legacy-load без автоопераций/миграции только на явной записи/оплаченных циклов. **Без UI/autoscheduling.** Сцена/режимы/авторизация/scheduler/Resume/приватность, переход на новый менеджер и сквозная приёмка — позже. До интеграции не разрешать сцене молча читать AI-run как local или записывать control, который она не исполняет.
...
S3.31 принят только в этих API-границах, [отчёт](../reports/2026-09-10/06-campaign-run-api.md). Ручной AI/save1/1 в игре, policy expansion-v1, механика/схемы CampaignSession/ShipDesign и зависимости прежние. На том этапе save2 ещё не существовал; его чистый кодек добавлен S3.32 ниже. UI постоянной стороны, репозиторий run, scheduling, победа и весь S3 не считаются реализованными.
...
Принят только кодек/миграция; новый API не подключён к сцене S3.29/save1/1. Следующий только S3.33 — репозиторий раздела9 без UI/autoscheduling. Чистые JSON checkpoints не заменяют будущую сценовую/storage/browser приёмку controller.
