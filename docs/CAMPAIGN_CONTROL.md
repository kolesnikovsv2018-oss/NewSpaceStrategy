...
- Статус: **только проектирование**. В работающей игре остаётся ручной AI S3.29 и save1/1. Типы, режимы, автомат и формат2 ниже ещё не реализованы.
...
- [MainScene](../src/scenes/MainScene.ts) сейчас исполняет синхронные ручные команды и отдельно атомарный executeAiTurn. PendingOperation/source/generation/disposed защищают подтверждения, но **ещё не существуют** ни авторизация постоянной стороны, ни отложенный AI-ticket. Нельзя считать старый pending готовым планировщиком.
...
Выбраны следующие **планируемые** типы, не текущие экспорты:
...
| RunResult | Успех содержит detached run и прежнюю квитанцию либо AI-summary; отказ не содержит run/частичного state/summary |
...
Следующий чистый API должен предоставить createCampaignRun(control), executeRunCommand(unknown run, unknown command), executeRunAiTurn(unknown run, unknown request) и convertRunToLocal(unknown run). Создание и преобразование возвращают проверяемый result, не бросают штатную validation-ошибку. Все входные объекты отделены и неизменны.

- executeRunCommand: полная run-схема → строгая прежняя command-схема → stale → active → контроллер human → прежний executeSessionCommand. Только потом действие/лимиты. В human-vs-ai любая ручная red-команда получает FACTION_CONTROLLED_BY_AI, даже если UI-кнопка/наблюдение подделаны. Local сохраняет прежние правила обеих сторон. Schema/refinement-исключения — INVALID_STATE без подробностей.
- executeRunAiTurn: полная run-схема → прежняя strict request{factionId,expectedTurn} → stale → active → разрешение → executeAiTurn. В local обе стороны доступны явному помощнику; в human-vs-ai только red, blue получает AI_NOT_ASSIGNED. Автоматическое/ручное разрешение запуска различает сцена, не новый флаг caller. Отказ executor сохраняется, успех возвращает run с прежним control и detached summary. Установка controller не даёт права обходить session validation.
...
Эта матрица — **ещё не выполненные тесты controller**. Имеющиеся1793 теста проверяют лишь игру до S3.29.
...
**S3.31 — следующий ограниченный шаг:** только чистые CampaignControl/CampaignRun и авторизованные run-команды/AI/takeover с регрессиями, без Phaser/scheduler/UI/смены работающего save. Прежние session/AI API и браузерное поведение остаются доступными как раньше. Не объединять будущие интеграции в этот шаг.

После него отдельными ограниченными задачами: новый run-кодек и явная1→2 миграция; run-репозиторий; сцена/режимы/авторизация/scheduler/Resume/приватность и переход на новый менеджер; сквозная приёмка. До интеграции controller не разрешать сцене молча читать AI-run как local или записывать control, который она не исполняет. Конкретные номера и границы уточнять после S3.31, не считать все задачи уже реализованными.

S3.30 закрывается наличием выбранного контракта режимов, прав, конечного scheduling, отказов/паузы/совместимости, матрицы и следующего чистого шага; **не наличием runtime**. В этой задаче src/tests/зависимости не изменены. Повторные test/typecheck/build подтверждают только исходную S3.29; браузерная приёмка controller предстоит.
