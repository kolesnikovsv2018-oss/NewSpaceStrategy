...
import { calculateComponent, componentSchema, COMPONENT_NAMES, createComponent, componentStatus,
...
const parameters: Record<ComponentKind, Parameter[]> = {
  beam: [
    ...
    { key: 'range', label: 'Дальность, такт. ед.', step: 50, min: 1, max: 10000 },
  ...
  engine: [
    ...
    { key: 'maxSpeed', label: 'Скорость, такт. ед/с', step: 10, min: 0, max: 1000 },
    { key: 'maneuverability', label: 'Манёвренность', step: 0.05, min: 0, max: 1 },
    { key: 'powerGeneration', label: 'Генерация, ЭЕ/с', step: 50, min: 0, max: 100000 }
  ],
  shield: [
  ...
  mining: [
    { key: 'miningSpeed', label: 'Номинал добычи, ед/с', step: 1, min: 0, max: 100000 },
    { key: 'efficiency', label: 'Эффективность', step: 0.05, min: 0, max: 1 }
  ],
  repair: [{ key: 'repairRate', label: 'Номинал ремонта, HP/с', step: 1, min: 0, max: 100000 }],
  scanner: [
    { key: 'range', label: 'Радиус, такт. ед.', step: 50, min: 0, max: 100000 },
    { key: 'accuracy', label: 'Точность', step: 0.05, min: 0, max: 1 }
  ],
  cargoExpansion: [{ key: 'bonusCapacity', label: 'Доп. объём, м³', step: 10, min: 0, max: 100000 }],
  armor: [
...
export class ComponentBuilderPanel {
  ...
  private render(): void {
    ...
    (Object.keys(COMPONENT_NAMES) as ComponentKind[]).forEach((kind, index) => {
      button(this.scene, this.container, 14 + (index % 5) * 84, 70 + Math.floor(index / 5) * 34, COMPONENT_NAMES[kind], () => {
    ...
    button(this.scene, this.container, 18, 143, '+ Создать', () => {
    ...
    filtered.slice(this.page * 4, this.page * 4 + 4).forEach((component, index) => {
      button(this.scene, this.container, 18, 180 + index * 30, component.name, () => {
        this.selectedId = component.id; this.render();
      }, `component-${component.id}`).setFixedSize(400, 28)
    ...
    text(this.scene, this.container, 18, 346,
      ...
      `Номинал: ${stats.power.toFixed(1)} ЭЕ/с · Выстрел: ${stats.energyPerShot.toFixed(1)} ЭЕ`, 12);
    text(this.scene, this.container, 18, 552, componentStatus(selected), 11, '#ffc880').setWordWrapWidth(400);
